...
int main()
{
    ...
    int j = 0; // Індекс для зберігання непозитивних елементів
    for (int i = 0; i < N; ++i) {
        if (A[i] < 0) {
            A[j] = A[i]; // Зберігаємо непозитивний елемент
            ++j; // Збільшуємо індекс для зберігання наступного непозитивного елемента
        }
    }
